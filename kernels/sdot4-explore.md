...
| **W4A8** (later) | W is i4, A is i8. Unpack nibble → i8, then `sdot4`. | Same 64×64×64 after unpack. `sdot8` (`V_DOT8_I32_I4`) is **W4A4 only** — both sides i4. Do not use `sdot8` for W4A8. | this page |
...
| mxfp4 / NVFP4 | E2M1 unpack → fp16 → `fdot2`. |
...
Pack W K-contiguous, `K % 8 == 0` (two sdot4 per dword of W). Same LDS seed as W8A8 after the unpack. No `sdot8` unless someone later ships W4A4 and accepts the numerics.

## Cards

Reuse the existing W8A8 INT8 and Sage cards (spec / Todo). Add **one** explore card: W4A8 `sdot4`. Do not clone a card per “no” row.
