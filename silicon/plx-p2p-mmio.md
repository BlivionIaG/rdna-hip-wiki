...
| Link | Payload |
|---|---|
| Gen4 x16 (88096 + V620/W7800, `LnkSta` 16 GT/s) | **31.508 GB/s** |
| Gen4 x8 | **15.754 GB/s** |
| Gen3 x16 (**V340L endpoint**, or 8749, or a gen-dropped 88096 port) | **15.754 GB/s** |
| Gen3 x8 | **7.877 GB/s** |

A V620 **slot** is Gen4 x16. A V340L **endpoint** is Gen3 — on these 88096 DSPs `LnkCap` stays Gen4, **`LnkSta` Speed = 8 GT/s**. That is the card, not the switch.

| Mesh | Lanes | Fits? |
|---|---|---|
| One 5-slot board: CPU x16 + 5× GPU x16 | 96 | **exact**, this hardware |
| 4× V620 x16 on one board (TP=4 PIX) | 80 | yes, one slot free |
| 8× V620 x16 on two boards (5+3 or 4+4) | 2×96 | yes; **PIX ≠ cross-board** |
| 2× W7800 + 8× V620 all x16 (5+5) | 2×96 | **yes now** (was 176 on one chip) |
| 8× V340L + 8× V620 | 16 slots | **no** — 10 slots; V340L is own host anyway |
| One 8749 (48 lanes) | — | cannot do 5× x16 |

`lspci -vv` **LnkSta** (negotiated), not **LnkCap**.

## Same-host ban: gfx1030 + gfx900

Do **not** put V340L (gfx900) in the V620 / 88096 host.

- Toolbox / amdgpu: a **pre-gfx1030** AMD card in the same box breaks HSA remapped MMIO (`Failed to map remapped mmio page`). V340L is Vega 10. Details: [v340l.md](v340l.md).
- ROCm 7.2 official Linux table includes **V620**, not Vega 10. Mixed `amdgpu` + KFD on one IOMMU domain is the trap, not a BAR-size tweak.
- Even if MMIO mapped: V340L is two HIP devices per slot, Gen3 `LnkSta`, **no** `fdot2`/`sdot4`. It does not join the PIX mesh as a 9th/10th V620.
- Slot count: 10 ≠ 16.

Own host, own ROCm line, own gfx900 `mad_mix` objects. Later.

## Mode we want vs modes we ignore

**Use — single-root transparent (Base Mode)**

- One CPU root → one upstream port → 5 downstream GPU ports **per board**.
...
- Peer MWr/MRd stay **inside** that board's switch (cut-through, PIX).
...
| **Cascaded 88096** | Later. Extra hop → `PXB`. Measure one-board PIX first. Default two-board install is two USPs = **`PHB`**. |
...
**Pass:** every PEX port that sits above a GPU shows `ACSCtl: SrcValid- …` and `setpci … ECAP_ACS+0x6.w` reads **`0000`**. Clear **both** 88096s — ACS on either board turns that board's PIX into a host bounce.
...
| **BAR5** | 32-bit non-prefetchable | **MMIO remap** | < 4 GB. Host/driver. **Pre-gfx1030 cards in this host break this map.** |
...
| Cards | BAR0 only |
|---|---|
| 4× V620 (one-board PIX) | **128 GiB** |
| 8× V620 | **256 GiB** |
| 2× W7800 | **96 GiB** |
| 2× W7800 + 8× V620 | **352 GiB** |

Plus doorbells + two PEX windows + NIC. BIOS: **Above 4G Decoding on**, **ReBAR / Resizable BAR on**, **CSM off**, **MMIO High Base/Size** so every BAR0 start is **< 2^44** (16 TiB). Kernel helper: `pci=realloc`.

`/proc/iomem` must show the `32G`/`48G` `amdgpu` windows above 4G, not a 256M clip.

Peer path: GPU A SDMA/shader issues **MWr/MRd** against GPU B **BAR0 bus address**. Switch routes TLP DSP→DSP **on that board**. ACS `SrcValid+` turns that into a host bounce. Cross-board without a cascade walks the CPU RP (`PHB`). Switch DMA is never in this path.
...
```bash
lspci -tv                                 # two PEX trees, 5 GPUs under each
...
# ACS control words — both switches
for BDF in $(lspci -d ::0604 | awk '{print $1}'); do
...
- `hipDeviceCanAccessPeer` all 1s **inside one board**; cross-board is a separate matrix
- `HSA_FORCE_FINE_GRAIN_PCIE=1`, `amdgpu.pcie_p2p=1`
- `NCCL_P2P_LEVEL=PIX` for a 4-GPU job on one board; `PHB` if ranks span both USPs; `PXB` only if cascaded
- `all_reduce_perf` + pairwise `hipMemcpyPeer` **ACS clear vs ACS on**, same-board vs cross-board
...
- **V340L in an 88096 slot:** expect `LnkSta` **8 GT/s x16** (endpoint Gen3). Not a board defect.
- 8749 is Gen3 only. Putting a V620 under it **is** the gen drop. These backplanes are 88096 — 8749 is the other SKU we documented, not this install.
