...
Live human branch: **`rdna2_extras`** @ **`e054854f`** (GDN delta_h 128-thr / 0 LDS; spill cut, not FA). Occupancy leftover unchanged: FA prefill still `(N, 1)`. Decode pin is off: FA 128/256 `__launch_bounds__(N)` + `amdgpu_waves_per_eu(4, 8)`; skinny dropped `(1, 1)`. Prefill FA still `__launch_bounds__(N, 1)`. Merge +1% on Qwen3.8-27B-AWQ 16k/1k TP=4 is **noise** — not a measured occupancy win. GPU occupancy query still TBD.
...
`gdn_decode_rdna2` + five `gdn_prefill_*`: all `(2, 4)`, not `(1,1)`. kkt stays scalar FMA. wy ~58 KB. delta_h @ `e054854f` is 128-thr, 0 LDS (was 256-thr / 64 KB ELF). Not the FA occupancy leftover. See [../kernels/gdn-prefill.md](../kernels/gdn-prefill.md).
